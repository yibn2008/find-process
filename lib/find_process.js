...
const finders = {
  ...
  win32 (cond) {
    return new Promise((resolve, reject) => {
      let executable = 'cmd'
      let cmd = 'WMIC path win32_process get Name,Processid,ParentProcessId,Commandline,ExecutablePath /format:list'
      let delimiter = '='
      const lines = []

      if (utils.hasPowerShell3()) {
        executable = 'powershell.exe'
        cmd = 'Get-CimInstance -className win32_process | select Name,ProcessId,ParentProcessId,CommandLine,ExecutablePath'
        delimiter = ':'
      }

      const proc = utils.spawn(executable, ['/c', cmd], { detached: false, windowsHide: true })
      proc.stdout.on('data', data => {
        lines.push(data.toString().replaceAll('\x00', '')) // WMIC add \x00
      })
      proc.on('close', code => {
        if (code !== 0) {
          return reject(new Error('Command \'' + cmd + '\' terminated with code: ' + code))
        }
        const list = utils.parseList(lines.join(''), delimiter)
          .filter(row => {
            if ('pid' in cond) {
