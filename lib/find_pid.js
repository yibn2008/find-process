...
* @Last Modified time: 2016-01-24 14:59:13
*/
...
const finders = {
  darwin (port) {
    return new Promise((resolve, reject) => {
      cp.exec("netstat -anv", function (err, stdout, stderr) {
        if (err) {
          reject(err);
        } else {
          ...
          let columns = utils.extractColumns(data, [0, 3, 8], 10).find(column => {
            if (!String(column[0]).match(/^(udp|tcp)/)) {
              return false;
            }

            let matches = String(column[1]).match(/\.(\d+)$/);
            if (matches && matches[2] === String(port)) {
              return true;
            }
          });
